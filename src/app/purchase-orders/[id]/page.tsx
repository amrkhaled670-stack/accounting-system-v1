...
export default async function PurchaseOrderDetailPage({
  ...
  const statusLabels: Record<string, { label: string; css: string }> = {
    draft: { label: t(locale, "po.draft"), css: "badge badge-draft" },
    confirmed: { label: t(locale, "po.confirmed"), css: "badge badge-confirmed" },
    received: { label: t(locale, "po.received"), css: "badge badge-paid" },
    cancelled: { label: t(locale, "po.cancelled"), css: "badge badge-cancelled" },
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t(locale, "po.detailTitle")} {order.orderNumber}</h1>
          <p className="text-muted text-sm mt-1">{t(locale, "po.supplierLabel")} {order.supplier.name}</p>
        </div>
        <Link href="/purchase-orders" className="text-accent hover:text-accent-hover text-sm">
          {arrow} {t(locale, "po.backToList")}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="stat-card">
          ...
          <p className="text-xl font-bold mt-1 font-mono">{order.totalAmount.toFixed(2)}</p>
          <p className="text-xs text-muted">
            {t(locale, "common.subtotal")}: {order.subtotal.toFixed(2)} | {t(locale, "common.tax")}: {order.taxAmount.toFixed(2)}
          </p>
        </div>
        <div className="stat-card">
          <p className="text-sm text-muted">{t(locale, "common.paid")}</p>
          <p className="text-xl font-bold mt-1 text-success font-mono">{order.paidAmount.toFixed(2)}</p>
        </div>
        <div className="stat-card">
      ...
      <h2 className="text-lg font-semibold mb-3 text-foreground">{t(locale, "po.orderItems")}</h2>
      <div className="overflow-x-auto mb-8 rounded-xl border border-border bg-card">
        <table className="table-pro">
          <thead>
            <tr>
              <th>{t(locale, "po.th_item")}</th>
              <th>{t(locale, "po.th_quantity")}</th>
              <th>{t(locale, "po.th_unitPrice")}</th>
              <th>{t(locale, "po.th_taxRate")}</th>
              <th>{t(locale, "po.th_taxAmount")}</th>
              <th>{t(locale, "po.th_lineTotal")}</th>
            </tr>
          </thead>
          <tbody>
      ...
      {order.taxExempt && (
        <p className="text-sm text-amber-600 mb-4">{t(locale, "po.taxExemptWarning")}</p>
      )}
      ...
      {order.payments.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold mb-3 text-foreground">{t(locale, "po.payments")}</h2>
      ...
      <POActions orderId={order.id} status={order.status} remaining={remaining} />
    </div>
  );
}
